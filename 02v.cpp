...
void Skaitymas (vector <duomenys> &A)
{
    ...
    if (D.good())
    {
        while (getline (D, line))
        {
            ...
            if (kiek > 0)
            {
                Egzaminas = Balai[kiek - 1];
                A[Nr].Vidurkio_Balas = 1.0 * (0.4 * Vidurkis (Balai, kiek - 1) + 0.6 * Egzaminas);
                A[Nr].Mediana = Mediana (Balai, kiek - 1);
            }
...
void Ivedimas (vector <duomenys> &A)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        A[i].Vidurkio_Balas = 1.0 * (0.4 * Vidurkis (Balai, n) + 0.6 * Egzaminas);
        A[i].Mediana = Mediana (Balai, n);
    }
    int r;
